...
const createCartService = (loginUserId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (colorId) {
        ...
        const associateWithProduct = yield Product_model_1.default.findOne({
            _id: productId,
            colors: { $in: [new ObjectId_1.default(colorId)] }
        });
        if (!associateWithProduct) {
            throw new ApiError_1.default(409, 'This colorId is not associated with this product');
        }
    }
    //check size
    if (sizeId) {
        const size = yield Size_model_1.default.findById(sizeId);
        if (!size) {
            throw new ApiError_1.default(404, "sizeId not found");
        }
        ...
        if (!associateWithProduct) {
            throw new ApiError_1.default(409, 'This sizeId is not associated with this product');
        }
        //set size
    ...
    const result = yield Cart_model_1.default.create(Object.assign(Object.assign({}, payload), { userId: loginUserId, name: product.name, price: product.currentPrice, image: product.images[0] }));
    return result;
});
